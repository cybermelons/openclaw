...
// modes keep the legacy value-compare. Shape parity is satisfied by always
...
// fixtures) and are flag-invariant, so each runs once under default (flag
// off) and once under flag on to prove the throw type doesn't regress either
// way.
...
const originalCasValueCompare = process.env.OPENCLAW_SESSION_CAS_VALUE_COMPARE;
...
for (const [modeLabel, casValueCompareEnabled] of [
  ["flag off (default)", false],
  ["flag on (OPENCLAW_SESSION_CAS_VALUE_COMPARE=1)", true],
] as const) {
  describe(`session lifecycle-state entry-CAS-snapshot-structural (T-P1h) — ${modeLabel}`, () => {
    let tempDir: string;
    let storePath: string;

    beforeEach(() => {
      if (casValueCompareEnabled) {
        process.env.OPENCLAW_SESSION_CAS_VALUE_COMPARE = "1";
      } else {
        delete process.env.OPENCLAW_SESSION_CAS_VALUE_COMPARE;
      }
    ...
    afterEach(() => {
      ...
      if (originalCasValueCompare === undefined) {
        delete process.env.OPENCLAW_SESSION_CAS_VALUE_COMPARE;
      } else {
        process.env.OPENCLAW_SESSION_CAS_VALUE_COMPARE = originalCasValueCompare;
      }
    });
