...
// expected/actual revision — green in both flag-off (default, revision
// compare) and flag-on (OPENCLAW_SESSION_CAS_VALUE_COMPARE=1, legacy
// value-compare) modes, per PHASE-1.md §7 shape-parity.
...
for (const [modeLabel, casValueCompareEnabled] of [
  ["flag off (default, revision compare)", false],
  ["flag on (OPENCLAW_SESSION_CAS_VALUE_COMPARE=1, legacy value compare)", true],
] as const) {
  describe(`session projection entry-CAS (T-P1g) — ${modeLabel}`, () => {
    let tempDir: string;
