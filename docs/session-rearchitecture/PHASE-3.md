...
Build `tmp_phase3-divergence-census.ts`. For every row: parse the blob with `parseSessionEntryBlob`, project every field, compare against every projected column. Output: per-field divergence count plus up to 10 sample keys per field. Corrupt rows go through the Phase 2 quarantine path and count separately. The census runs three times in this phase: before re-routing (detect class c), before the divergence-check deletion (gate for CS-6), and before the column drop (gate for CS-7). The census makes the silent check's hidden data visible one last time before the check dies.

---

## 3. Divergence-Check + entry_valid-Trigger Deletion

### 3.1 Precondition

ALL class (b) readers are re-routed and landed. ALL class (c) fields are resolved and their readers re-routed. The audit table shows zero remaining fact-reads. The Phase 2 lint fence is extended (CS-5) so a new fact-read cannot appear.

Deletion order matters: **the divergence check dies only AFTER the last reader re-route.** While one fact-reader remains, the check still guards a real observable condition. Deleting it first hides a live bug. See §8(d).

### 3.2 Gate for the deletion changeset

1. Audit table: every row is class (a), or class (b) with a landed re-route commit hash, or class (c) with a landed resolution commit hash.
2. Divergence census run 2: zero divergence on every field. If nonzero, a writer is still bypassing the canonical path. Find it. Do not delete.
3. Full test wall green (except T-A3a, red-pinned).
4. The `entry_valid` trigger deletion ships in the SAME changeset as the divergence-check deletion (CS-6). Both guard the same dead condition. The compensating-UPDATE code paths are deleted with the trigger.
...
| CS-6 | Delete divergence check + entry_valid trigger | Delete `session-entry-json.ts:26-31` check. Delete `entry_valid` trigger and compensating-UPDATE code. Census run 2 immediately before landing.                                                                                                 | CS-5                                            | §3.2 gate in full. Zero divergence on census run 2.                                                                                   |
| CS-7 | Subtractive migration — column drop           | Archive-then-drop migration (§4.3). Write path stops writing dropped columns. Lands only after the soak window.                                                                                                                                 | CS-6 + soak (≥ one release, zero tripwire hits) | Migration tests green (§6.4). Census run 3 zero. Soak log zero. Orchestrator sign-off recorded — this is the irreversible step.       |
...
**(d) Divergence check deleted before a reader is re-routed — hides a real bug.**
Attack: CS-6 lands early "because the check is annoying," while one class (b) reader still reads the column. Now that reader consumes divergent values with no alarm at all — strictly worse than the Phase 0 status quo.
Detection: the CS-6 gate (§3.2) is mechanical, not judgment: the audit table must show a landed commit hash for every class (b) and (c) row, and census run 2 must be zero, taken immediately before landing. A CS-6 that lands without both artifacts fails its own gate definition.
Ordering guard: the dependency chain CS-6 → CS-5 → CS-3/CS-4 → CS-2 → CS-1 is stated in §5 and is not reorderable: each gate consumes an artifact (audit hashes, census output, green pins) that only the prior CS produces. The check's deletion is not a cleanup that can float — it is the reward for a completed audit, and the plan withholds it until the audit is complete.
